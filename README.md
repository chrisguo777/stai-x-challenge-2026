...
>
> 下一发候选:**调参后的 HistGB(absolute_error)+ RandomForest 的 0.7/0.3 集成**(本地 5 折 1.7086,优于 0.5/0.5 平均 1.7125、单默认 HistGB 1.7247),已封装进 `kaggle_submission.ipynb`,待提交验证榜分。
...
- **新增三个上分方向的验证(均在 5 折 GroupKFold-by-period、仅对 3 个计分类别评分)**:
  - `13_multitask_categories.py`:把 5 个非计分类别当额外训练行(多任务)。结论 **否决**——多任务 1.7200 > 基线 1.7125,不同类别的率量纲差异太大反而干扰。
  - `14_tuning_and_loss.py`:MAE 对齐损失 + 轻调参 + 集成权重扫描。结论 **采用两项**——单纯换 `absolute_error` 反而更差(1.7319),但配合调参(learning_rate=0.05, max_iter=600 等)后 1.7206;集成权重从 0.5/0.5 调到 **0.7/0.3** 后 **1.7086**(最干净的赢点)。
  - `15_text_tfidf.py`:折内 fit 的 TF-IDF(→ TruncatedSVD)文本特征。结论 **否决**——1.7695 > 1.7362,`all_drugs` 也更差,文本对该任务始终无可利用信号。
...
- `13_multitask_categories.py`:多任务(8 类别)训练。**否决**(1.7200 > 1.7125)。
- `14_tuning_and_loss.py`:MAE 损失 + 调参 + 集成权重。**采用** abs-tuned HGB(1.7206)+ 权重 0.7/0.3(1.7086)。
- `15_text_tfidf.py`:折内 TF-IDF 文本特征。**否决**(1.7695 > 1.7362)。

三者均在 5 折 GroupKFold-by-period、仅对 3 个计分类别评分,只有 14 的两项小改通过门槛并已并入提交脚本。

### `notebooks/kaggle_submission.ipynb`(提交用)

作用：**自包含的 Code-Competition 提交 notebook**。从原始竞赛数据现算图像特征、Dataset C 填补、训练「调参后 HistGB(absolute_error)+ RandomForest 的 0.7/0.3 集成」、写出 `/kaggle/working/submission.csv`。不依赖本地 `outputs/` 或 01-15 / `statx_helpers.py`——所需逻辑全部内联,可在隐藏 later window 上独立重跑。对应脚本 `kaggle_submission.py`(两者同源,本地可用 `STAIX_DATA=. python notebooks/kaggle_submission.py` 测试)。
...
试过两种:**关键词计数**(notebook 09/10)和**折内 fit 的 TF-IDF→SVD**(notebook 15)。两者在 5 折下都**没有增益甚至变差**(TF-IDF 1.7695 > 无文本 1.7362,`all_drugs` 也更糟)。`has_doh_release` 已捕获"有无发文",更细的文本信息没带来稳定可用的信号。故最终模型**不含文本特征**。原先以为文本是最大潜在增量,但 TF-IDF 这一更强 NLP 方法证伪了这个假设。

> 各类误差不均:`all_drugs`(率高)MAE 3.09 最难,`all_opioids` 1.39,`all_stimulants` 0.73 最好预测。后续想再上分,重点在啃 `all_drugs`。

### 最后两项小改:MAE 损失调参 + 集成权重

在选定 universal + Dataset C + 图像后,又验证了三个上分方向(notebook 13/14/15),只有两项通过 5 折门槛,已并入提交脚本:

| 改动 | 5 折 scored MAE | 判定 |
|---|---|---|
| 基线:HGB+RF 各 0.5 平均 | 1.7125 | — |
| 多任务(8 类别训练) | 1.7200 | ❌ 否决 |
| 纯 `absolute_error` 损失 | 1.7319 | ❌ 单换损失反而更差 |
| **`absolute_error` + 调参** | **1.7206** | ✅ 优于单默认 HGB 1.7247 |
| TF-IDF 文本 | 1.7695 | ❌ 否决 |
| **集成权重 0.7/0.3** | **1.7086** | ✅ 最干净的赢点 |

最终提交模型 = **调参后 HGB(absolute_error, lr=0.05, max_iter=600)× 0.7 + RandomForest × 0.3**,本地 5 折 **1.7086**。
